...
# Generate AWS ParallelCluster configuration from Terraform outputs
# This script reads Terraform outputs and generates a cluster-config.yaml file
...
REGION=$(terraform output -raw pcluster_config_values | jq -r '.region')
HEAD_NODE_SUBNET_ID=$(terraform output -raw pcluster_config_values | jq -r '.head_node_subnet_id')
COMPUTE_SUBNET_ID=$(terraform output -raw pcluster_config_values | jq -r '.compute_subnet_ids[0]')
SECURITY_GROUP_ID=$(terraform output -raw pcluster_config_values | jq -r '.security_group_id')
EFS_FILE_SYSTEM_ID=$(terraform output -raw pcluster_config_values | jq -r '.efs_file_system_id')
CUSTOM_AMI=$(terraform output -raw pcluster_config_values | jq -r '.custom_ami')
# EFS_ACCESS_POINT_ID removed - no longer using access points

# Validate that we got all required values
if [[ -z "$REGION" || "$REGION" == "null" ]]; then
    echo "Error: Could not retrieve region from Terraform outputs"
    exit 1
fi

if [[ -z "$HEAD_NODE_SUBNET_ID" || "$HEAD_NODE_SUBNET_ID" == "null" ]]; then
    echo "Error: Could not retrieve head node subnet ID from Terraform outputs"
    exit 1
fi

if [[ -z "$COMPUTE_SUBNET_ID" || "$COMPUTE_SUBNET_ID" == "null" ]]; then
...
if [[ -z "$EFS_FILE_SYSTEM_ID" || "$EFS_FILE_SYSTEM_ID" == "null" ]]; then
...
if [[ -n "$CUSTOM_AMI" && "$CUSTOM_AMI" != "null" ]]; then
